...
Three buckets, and only the third reaches a human:
  ...
  3. **Names a different title.** Every one of these needs a `keep` or `drop` decision. One with no
     decision REFUSES the publish, the way `check_producers.py` refuses an artifact with no producer: new
...
output comes from Wikidata.
    ...
    pipeline/check_alias_collisions.py out-facts-full/facts-<hash>.json
    pipeline/check_alias_collisions.py <facts.json> --tmdb --review out/alias-review.json
    pipeline/check_alias_collisions.py --gate out/dataset.meta.json
"""
import argparse
import hashlib
import json
...
def gate(meta_path, decisions_path):
    ...
    if record["undecided"]:
        ...
        print(f"error: the store ships {record['undecided']} alias(es) that are another title's name, with no "
              ...
              f"         1. list them:\n"
              f"              pipeline/check_alias_collisions.py {facts} --review alias-review.json\n"
...
def main():
    ...
    if residue and args.review:
        if args.tmdb:
            ...
            if not key:
                sys.exit("--tmdb needs TMDB_API_KEY (lib/den-env.sh loads den.env)")
