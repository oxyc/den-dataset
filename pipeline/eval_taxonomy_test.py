"""`eval_taxonomy.py` — the only check that measures label QUALITY rather than plumbing.
...
def load():
    spec = importlib.util.spec_from_file_location("eval_taxonomy", os.path.join(HERE, "eval_taxonomy.py"))
...
class Gate(unittest.TestCase):
    ...
    def _run(self, labels_rows, extra=()):
        ...
        argv = sys.argv
        sys.argv = ["eval_taxonomy.py", lpath, "--golden", self.golden, "--floors", self.floors, *extra]
...
class CommittedFloors(unittest.TestCase):
    ...
    def test_the_committed_genres_and_moods_are_what_the_baseline_was_recorded_on_and_pass_it(self):
        ...
        self.assertEqual(recorded["labelsSha256"], ev.sha256(curated),
                         ...
                         "against it")
        argv = sys.argv
        sys.argv = ["eval_taxonomy.py", curated, "--golden",
