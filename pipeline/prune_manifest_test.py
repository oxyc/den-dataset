#!/usr/bin/env python3
"""`prune_manifest.py` — the rule that decides what `data-latest` still declares."""
...
def load():
    ...
    spec = importlib.util.spec_from_file_location("prune_manifest",
                                                  os.path.join(HERE, "prune_manifest.py"))
...
def pruned(meta):
    ...
    with tempfile.TemporaryDirectory() as dir:
        ...
        with open(path, "w") as fh:
            json.dump(meta, fh)
        old = sys.argv
        sys.argv = ["prune_manifest.py", "--prune", path]
        try:
            pm.main()
        finally:
            sys.argv = old
...
class PruneManifest(unittest.TestCase):
    ...
    def test_nothing_describes_a_file_the_manifest_does_not_name(self):
        ...
        contradicts itself — and `manifest_counts.py --consistent` would refuse the publish for it.
        """
        after = pruned(LIVE)
