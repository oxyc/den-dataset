"""`check_filename_version.py` — refuse a blob from a dead `datasetVersion`.
...
def load():
    spec = importlib.util.spec_from_file_location(
        "check_filename_version", os.path.join(HERE, "check_filename_version.py"))
...
class Stale(unittest.TestCase):
    ...
    def test_main_exits_nonzero_on_a_stale_blob(self):
        with tempfile.TemporaryDirectory() as dir:
            ...
            import sys
            old = sys.argv
            sys.argv = ["check_filename_version.py", path]
            try:
