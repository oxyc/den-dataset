...
python3 "$HERE/sign_manifest.py" keygen "$KEYDIR/key.pem" >/dev/null || { echo "keygen failed" >&2; exit 1; }
export DEN_DATASET_SIGNING_KEY="$KEYDIR/key.pem"
DEN_DATASET_PUBLIC_KEY="$(python3 "$HERE/sign_manifest.py" public-key)" || { echo "public-key failed" >&2; exit 1; }
export DEN_DATASET_PUBLIC_KEY

# Does the meta's signature verify, with openssl, against the scratch key's public half?
signature_verifies() {
  python3 - "$HERE/sign_manifest.py" "$1" "$DEN_DATASET_SIGNING_KEY" <<'PY'
...
write_meta() {
  ...
  # The record COUNT has to be real: `manifest_counts.py --consistent` compares what the manifest
...
MKLABELS
  ...
  # A REAL store-v1 header. `manifest_counts.py` reads `row_count` from the file rather than trusting
...
write_meta
publish_baseline
python3 "$HERE/sign_manifest.py" keygen "$WORK/other.pem" >/dev/null
...
#
# `datasetVersion` names the GENERATION, and check_filename_version.py requires every versioned filename
...
if run_publish; then
  ...
  grep -q "check_alias_collisions.py .* --review" "$WORK/err.log" && grep -q '"keep" or "drop"' "$WORK/err.log" \
