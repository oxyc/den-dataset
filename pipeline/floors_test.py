...
class Tiers(unittest.TestCase):
    ...
    def test_the_chosen_defaults(self):
        """Regional is the measured median (3); worldwide is raised above its median (5) to 10, because each
        admitted title also needs hand-run premise tags (see the module docstring)."""
        self.assertEqual(floors.DEFAULT, floors.Floors(tmdb=50, regional_tmdb=15, wikipedias=10,
...
class Given(unittest.TestCase):
    def test_a_named_floor_replaces_its_default_and_only_it(self):
        ...
        self.assertEqual(floors.given(regional_wikipedias=2), floors.Floors(50, 15, 10, 2))
