...
"""Cut `dataset.meta.json` down to the one artifact the release still carries.
    ...
    prune_manifest.py --prune   <meta.json>   # rewrite it, dropping every retired blob's keys
    prune_manifest.py --retired <meta.json>   # print the keys --prune would drop, one per line
...
none of BLOB_SUFFIXES, which is why it survives; `prune_manifest_test.py` pins that.
...
# exact failure `manifest_counts.py` was written about. `storeRecords` is the row count, and it is checked.
#
