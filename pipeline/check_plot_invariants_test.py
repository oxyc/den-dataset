"""`check_plot_invariants.py` — the shared-article grounding invariant (oxyc/den-dataset#16).
...
def load():
    ...
    spec = importlib.util.spec_from_file_location(
        "check_plot_invariants", os.path.join(HERE, "check_plot_invariants.py"))
...
def run(argv):
    ...
    import sys
    argv_was = sys.argv
    sys.argv = ["check_plot_invariants.py"] + argv
    try:
        with contextlib.redirect_stdout(out), contextlib.redirect_stderr(err):
            code = cpi.main()
    finally:
