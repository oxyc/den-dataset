...
#: The pipeline, in the order it runs. Classification comes first because it reads the articles and nothing
#: else here does; embedding next, because its stores are what `finalize` turns into the vectors the store
#: is built from; the corpus is joined before the store for the same reason; and publishing is last because
#: it uploads what the store wrote. That ordering is the whole reason a stage can stop naming a producer for
#: an artifact another stage makes.
#:
...
#:
#: Short because the port is five stages in. The rest — worklist, fetch — still live under `scripts/`, run
#: from `docs/OPERATE.md`, and join here one at a time (oxyc/den-dataset#27).
STAGES = ("classify", "embed", "corpus", "store", "publish")
