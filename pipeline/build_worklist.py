...
The universe = the TMDB ids Den already ships (extracted from the out-dir's `labels-t02.json`, the labels
the last `finalize` wrote), so a re-embed covers exactly the current catalogue. Ordering = TMDB daily-export `popularity` desc, so enrich processes the
...
wikiPlot rate fall off and stop when it's not worth continuing).
    ...
    python3 pipeline/build_worklist.py            # -> out/worklist-{movie,tv}.json

Env:
    LABELS  path to the shipped labels blob (default: $OUT_DIR/labels-t02.json)
...
# The shipped labels, where the pipeline keeps them: the out-dir, under the name `finalize` writes and the
# worklist stage reads as `vector_labels`. The default used to point into the den app's bundle, which den
...
def fetch_export(kind: str) -> str:
    ...
    for day_offset in (0, 1):  # today's export is generated ~08:00 UTC; fall back a day if not up yet
        ...
        url = f"https://files.tmdb.org/p/exports/{kind}_ids_{stamp}.json.gz"
        try:
            ...
            # Today's file not being up yet is the expected miss; anything else must still be visible.
