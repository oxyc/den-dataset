...
This is the same shape as the failures `check_producers.py` exists for: an artifact drifting from the thing
...
`--max-batch-id` bounds the comparison to the batches a publish actually saw; `manifest_counts.py --stamp`
records it as `maxBatchId`.

    pipeline/check_plot_invariants.py --enriched-dir out-t02/enriched --labels out-publish/labels-t02.json
    pipeline/check_plot_invariants.py ... --max-batch-id 177 --facts out-publish/facts-<ver>.json
    pipeline/check_plot_invariants.py ... --shared-plot-baseline 1066 --stamp-meta out/dataset.meta.json
    pipeline/check_plot_invariants.py ... --store out/den-<version>.store
...
def report_provenance(census, grounded, limit=10):
    ...
    if census["unrecorded"]:
        print(f"  no recorded provenance : {len(census['unrecorded'])} — enriched before the pass recorded "
              ...
              "      pipeline/backfill_plot_provenance.py --enriched-dir <dir> --out-dir <dir>")
    return len(found)
...
def wikidata_identity(facts_records):
    ...
    Ships as `one_or_many` (check_facts_schema.py): a bare string for nearly every title, a list where
...
def main():
    ...
    if args.stamp_meta:
        ...
        # `manifest_counts.py` stamps `maxBatchId` rather than adding it to `DatasetMeta`: a key the struct
    ...
    if args.shared_plot_baseline is not None and shared_titles > args.shared_plot_baseline:
        ...
        print("       Fix the grounding, or — if this increase is understood and deliberate — say why:",
              file=sys.stderr)
        print("         DEN_ALLOW_SHARED_PLOTS='why the count went up' pipeline/publish-dataset.sh <dir>",
              file=sys.stderr)
        return 2
