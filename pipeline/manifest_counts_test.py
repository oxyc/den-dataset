"""`manifest_counts.py` — the record-count stamp and the shrink guard.
...
def load():
    ...
    spec = importlib.util.spec_from_file_location("manifest_counts", os.path.join(HERE, "manifest_counts.py"))
...
def run(argv):
    ...
    out = io.StringIO()
    old = sys.argv
    sys.argv = ["manifest_counts.py", *argv]
    try:
