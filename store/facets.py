...
reaches them, and the `applic_*` columns the gates judge against.

den-spec `wire/store-v1.md` § "Facets". The store carries an argmax and one confidence byte per axis,
...
class Facets:
    ...
    def __init__(self):
        ...
        # What the publication gates published and withheld, per axis — reported at the end of the build
        # and stamped into the manifest. A gate that drops 42% of `ending` must say so in a number, not
        # leave the next reader to discover it as a coverage surprise.
        self.published = Counter()
        self.withheld = defaultdict(Counter)

    def intern(self, strings, row):
        """Interned under the SAME gate `add` applies, so a value no row publishes never reaches the
        dictionary. Interning it anyway would leave a vocabulary in the store that no row uses, which
        reads from the outside as a value with zero titles rather than as one the gate withheld."""
    ...
    def announce(self, rows):
        """Per axis, on stderr: published, withheld, and which clause did the refusing."""
        print("plot-facet publication gates (FACETS-V2 § Publication gates):", file=sys.stderr)
        for axis in FACET_AXES:
            kept = self.published[axis]
            why = ", ".join(f"{r}={c}" for r, c in self.withheld[axis].most_common())
            print(f"  {axis:12} {kept:6d}/{rows} ({kept / rows * 100:5.1f}%)  withheld: {why}",
                  file=sys.stderr)
    ...
    def report(self):
        ...
        return {
            ...
            "axes": {axis: {"published": self.published[axis],
