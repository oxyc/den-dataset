...
"""Score the published labels against the hand-labelled golden set, and gate a publish on it.
    ...
    scripts/eval-taxonomy.py out-repass/labels-t02.json --gate     # …and exit 1 below baseline - tolerance
    scripts/eval-taxonomy.py out-repass/labels-t02.json --record   # make these scores the baseline
...
# Two tests hold that end down — a shuffled mood in `scripts/test_eval_taxonomy.py` and a derive rule that
    ...
    if args.record:
        ...
        if drops and not args.accept_drop:
            ...
            print("       Recording a drop is a decision, not a side effect. If it is deliberate:\n"
                  f"         scripts/eval-taxonomy.py {args.labels} --record --accept-drop", file=sys.stderr)
            return WOULD_LOWER
        recorded = {
